...
SELECT ship_address, COUNT(ship_address) FROM orders
...
SELECT AVG(unique_shippers) FROM 
(SELECT customer_id, COUNT(DISTINCT ship_via) AS unique_shippers
FROM orders
...
FROM orders
GROUP BY customer_id)
SELECT AVG (unique_shippers) FROM sub;
...
SELECT u.state_name, u.state_abbr, c.company_name From customers c 
RIGHT JOIN us_states u ON u.state_abbr = c.region
ORDER BY u.state_name;

-- 3.4 Left/Right Join and then some
